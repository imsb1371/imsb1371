...
function Research() {
  ...
  const filtered = activeTag === 'All'
    ? allProjects
    : allProjects.filter(p => p.tags.some(t => t.toLowerCase().includes(activeTag.toLowerCase())))
    ...
    <div style={{ backgroundColor: '#F8FAFC', minHeight: '100vh' }}>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: '1100px', margin: '0 auto', position: 'relative' }}>
          ...
          <p style={{ color: '#94A3B8', fontSize: '1.05rem', maxWidth: '560px', lineHeight: 1.7, margin: 0 }}>
            Open-source tools, published systems, and ongoing explorations at the intersection
            of human-computer interaction and machine learning.
          </p>
        </div>
      </section>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '3.5rem 1.5rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.6rem', flexWrap: 'wrap', marginBottom: '2.5rem' }}>
          {allTags.map(tag => (
            <button
              ...
              onMouseOver={e => { if (activeTag !== tag) { (e.currentTarget as HTMLElement).style.borderColor = '#0D9488'; (e.currentTarget as HTMLElement).style.color = '#0D9488' } }}
              onMouseOut={e => { if (activeTag !== tag) { (e.currentTarget as HTMLElement).style.borderColor = '#E2E8F0'; (e.currentTarget as HTMLElement).style.color = '#475569' } }}
            >
              {tag}
            </button>
          ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '1.5rem' }}>
          {filtered.map((project, i) => {
            ...
            return (
              ...
              >
                {/* Card top accent */}
                <div style={{ height: '4px', background: `hsl(${(i * 60) % 360}, 65%, 45%)`, opacity: 0.7 }} />

                <div style={{ padding: '1.75rem', flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '1rem', marginBottom: '1rem' }}>
                    <h3 style={{ fontFamily: "'Playfair Display', Georgia, serif", fontSize: '1.1rem', fontWeight: 600, color: '#0F172A', lineHeight: 1.35, margin: 0 }}>
                      {project.title}
                    </h3>
                  </div>

                  <p style={{ color: '#475569', fontSize: '0.875rem', lineHeight: 1.7, flex: 1, marginBottom: '1.25rem' }}>
                    {project.description}
                  </p>
                  ...
                  <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', flexWrap: 'wrap' }}>
                    {project.github && (
                      <a
                        ...
                        style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', color: '#475569', fontSize: '0.8rem', fontWeight: 500, textDecoration: 'none', transition: 'color 0.2s' }}
                    ...
                    {project.liveUrl && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem', color: '#0D9488', fontSize: '0.8rem', fontWeight: 500, textDecoration: 'none', transition: 'color 0.2s' }}
                    ...
                    {project.content && (
                      <button
                        onClick={() => setExpandedProject(isExpanded ? null : project._meta.path)}
                        style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: '0.3rem', color: '#94A3B8', fontSize: '0.8rem', background: 'none', border: 'none', cursor: 'pointer', padding: 0, transition: 'color 0.2s' }}
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '4rem', color: '#94A3B8' }}>
            <p style={{ fontSize: '1rem' }}>No projects found for "{activeTag}"</p>
          </div>
        )}
      </div>
    </div>
